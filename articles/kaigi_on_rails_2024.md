...
- POROのユースケースやルール作りの話が勉強になったた
  - 「クラス名には返すオブジェクトの名前を名詞でつける」
- 複数のテーブルにまたがる複雑な処理の場合に安易にサービスクラスを作っていたが対応を考え直したい
...
- 処理が長い非同期ジョブを実装したことがないため、中断や再会処理を意識したことはないが実装方法が勉強になった
- Sidekiqを触ったPJ後に追加されたため、Sidekiq Iterationは初めて知った
- Sidekiqは複雑だが、機能が充実しているため、再度触る際にはドキュメントをしっかり読んで使いこなしたい
...
- 別のmeetupでスクリーンショットを元にLLMにブラウザを自動操作させる話を聞いたことがあり、その延長線上で理解できました
...
- The One Person Frameworkという概念を初めて知った
- ブログ記事に記述はあるが、「一人分の脳に収まる程度の情報量でアプリを記述できるように」という表現がわかりやすかった
- 「居住可能性」という表現も興味深かった
  - 心地よく自信を持って変更を加えられるもの
...
- 定例MTGと合わせて実施することで取り組みやすくする工夫やテンプレートを作成して議論を進めやすくする方法が参考になりました
...
自動テストの実行時間の短縮やデータマイグレーションの運用を見直すなど、開発者体験を向上させるための取り組みが多く見られました。

Rails Wayはただの開発手法ではなく、哲学であることを実感しました。
懇親会などで1日目の基調公演が基調講演として理想的だったということを話しましたが、2日目の発表でもこの基調公演の内容に早速触れられており、最後の基調講演でも再度触れられていたので、Rails WayがRailsの開発者にとって重要なものであることを改めて認識しました。
